...
<!--                        <span style="color: #1890ff; cursor: pointer" @click="findParent(record.parentId)">[查找]</span>-->
                        <router-link :to="{path: '/views/SiteVerifyV4?layoutId='+ record.parentId}" target='_blank'>[查找]</router-link>
                    </div>
                </div>
            </template>
            ...
            <template slot="operation" slot-scope="text, record, index">
                <div v-if="record.state == 1">
                    <template>
                        <a href="javascript:;" class="block" @click="templateOn(record, index)"
                           v-if="record.copyState != 1">上架模板</a>
                        <a href="javascript:;" class="block colorRed" @click="templateOff(record, index)"
                           v-else>下架模板</a>
                    </template>
                    <template>
                ...
                <a-spin :spinning="spinning" tip="加载中。。。">
                    <template>
                        ...
                        <div v-for="(item) in checkboxAll" :key="item.categoryId">
                            ...
                            <a-checkbox-group :key="checkedboxAll" name="name" :options="item.tagList"
                                              @change="checkboxChange" :default-value="checkedboxAll"/>
